...
# 가지치기도 할 수 있을거 같음(아닌가? 산 높이를 음수까지 뚫을 수 있어서 안되나?)
# 뭔가 남은 조건으로 만들 수 있는 가장 최선의 케이스를 썻는데도, 최대값이 안 만들어지는 경우를 만들 수 있을거 같음
# 1. (현재까지의 경로) + (현재 봉우리의 크기 -1) + (남은 k)가 지금의 최댓값 보다 작거나 같은 경우에 가지치기
# => k = 2, 봉우리 = 3일때 3, 2, 1, 0, -1 이렇게까지 만들 수 있으니깐... 이 아니네
# K를 1부터 계속 바닥으로 뚫게 하려면 1, 2, 3, 4... n 까지 해야할 거 같은데 1, 3, 6, 10, 15... 이걸 O(1)로 구하는 방법이 없을까?
# 일단 그냥 가지치기 없이 해보자
