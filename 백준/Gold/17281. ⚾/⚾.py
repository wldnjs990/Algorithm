...
# 조합을 구하는 문제인거 같은데 8! 정도는 그냥 구할 수 있나?
# 4만 정도 나오네
# 조합 구할때마다 얘네로 이닝을 돌려야 함

# 이닝 돌리는 법
# 이닝 배열 하나 받아서 아웃 카운트가 3이 쌓일때 까지 진행해야함
# 

# 이닝 돌리는거에서 시간초과 뜨는거 같은데
# 가지치기 할 수 있는 방법 없나..?
...
def makeTeam(lev=0):
  ...
  # gpt한테 물어보니 lev가 3인 경우에 따로 재귀를 실행시키고 밑에 코드를 실행시키지 말아야함
  if lev == 3:
      team[lev] = 0
      makeTeam(lev+1)
      return
...
def get_max():
   ...
   for inning in innings:
      # 투루(?)
      # 0번째는 대기존, 1, 2, 3루 까지만 존재(4루는 1점)
      st = False
      nd = False
      rd = False
      out = 0
      while out < 3:
        now_res = inning[team[now]]
        # 아웃이면 카운트 늘리고 다음으로
        if now_res == 0:
          out += 1
          now = (now + 1) % 9
